prediction_hg38 <- function(up,down){
        ...
        command <- c("intersectBed -wa -wb -a data/predictedTRUE_up_hg38 -b data/test_up|cut -f4,8 > data/PredictedAlupair")
        try(system(command))
        
        command <- c("intersectBed -wa -wb -a data/predictedTRUE_down_hg38 -b data/test_down|cut -f4,8 >> data/PredictedAlupair")
        try(system(command))
        ...
        if(file.info("data/PredictInPair")$size>0){
                ...
                try(system(command))
                command <- c("awk 'NR==FNR {h[$1]; next} $4 in h' data/temp data/predictedTRUE_up_hg38 |sort -n -k4>data/temp2")
                try(system(command))
                
                command <- c("awk 'NR==FNR {h[$1]; next} $4 in h' data/temp data/predictedTRUE_down_hg38 |sort -n -k4>data/temp3")
                try(system(command))
